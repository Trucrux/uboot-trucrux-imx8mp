...
#include "imx8mp_trux.h"

extern int trux_setup_mac(struct trux_eeprom *ep);
...
static iomux_v3_cfg_t const uart_pads_trux[] = {
...
int board_early_init_f(void)
{
	...
	board_id = trux_detect_board_id();
	if (board_id == BOARD_ID_TRUX) {
		imx_iomux_v3_setup_multiple_pads(uart_pads_trux,
			ARRAY_SIZE(uart_pads_trux));
		init_uart_clk(0);
	}
	else {
...
int trux_detect_board_id(void)
{
	...
	if (gpio_get_value(BOARD_DETECT_GPIO))
		...
		board_id = BOARD_ID_TRUX;
#else
	if (of_machine_is_compatible("trucrux,imx8mp-trux-som"))
		board_id = BOARD_ID_SOM;
	else if (of_machine_is_compatible("trucrux,imx8mp-trux"))
		board_id = BOARD_ID_TRUX;
#endif
...
#define EQOS_PWR_GPIO_TRUX IMX_GPIO_NR(2, 20)
#define EQOS_RST_GPIO_TRUX IMX_GPIO_NR(2, 11)

#define EQOS_PWR_GPIO_SOM IMX_GPIO_NR(2, 20)
#define EQOS_RST_GPIO_SOM IMX_GPIO_NR(1, 10)

static iomux_v3_cfg_t const eqos_pads_trux[] = {
...
static void setup_iomux_eqos(void)
{
	...
	int board_id = trux_detect_board_id();

	if (board_id == BOARD_ID_TRUX) {
		imx_iomux_v3_setup_multiple_pads(eqos_pads_trux,
					 ARRAY_SIZE(eqos_pads_trux));
		pwr_gpio = EQOS_PWR_GPIO_TRUX;
		rst_gpio = EQOS_RST_GPIO_TRUX;
	} else {
...
int board_late_init(void)
{
	...
	if (board_id == BOARD_ID_SOM) {
		env_set("board_name", "TRUX-SOM-MX8M-PLUS");
		env_set("console", "ttymxc1,115200");
	}
	else if (board_id == BOARD_ID_TRUX) {
		int carrier_rev = trux_detect_trux_carrier_rev();

		env_set("board_name", "TRUX-MX8M-PLUS");

		if (carrier_rev == TRUX_CARRIER_REV_2)
			env_set("trux_carrier_rev", "8MDVP-2.x");
		else
			env_set("trux_carrier_rev", "legacy");
	}
