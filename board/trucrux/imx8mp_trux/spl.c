...
struct i2c_pads_info i2c_pads_trux = {
	.scl = {
...
#define USDHC2_RST_GPIO_TRUX IMX_GPIO_NR(2, 19)
...
static iomux_v3_cfg_t const usdhc2_rst_pads_trux[] = {
	...
	for (i = 0; i < CONFIG_SYS_FSL_USDHC_NUM; i++) {
		...
		case 0:
			...
			if (board_id == BOARD_ID_TRUX) {
				rst_gpio = USDHC2_RST_GPIO_TRUX;
				imx_iomux_v3_setup_multiple_pads(
					usdhc2_rst_pads_trux,
					ARRAY_SIZE(usdhc2_rst_pads_trux));
			}
			else {
...
int power_init_board(void)
{
	...
	if (trux_detect_board_id() == BOARD_ID_TRUX)
...
{
	int id = trux_detect_board_id();

	if (id == BOARD_ID_TRUX) {
		int carrier_rev = trux_detect_trux_carrier_rev();

		if ((carrier_rev == TRUX_CARRIER_REV_1) &&
			!strcmp(name, "imx8mp-trux-8MDVP-legacy"))
			return 0;
		else if ((carrier_rev == TRUX_CARRIER_REV_2) &&
			!strcmp(name, "imx8mp-trux-8MDVP"))
			return 0;
	}
	else if ((id == BOARD_ID_SOM) && !strcmp(name, "imx8mp-trux-som-symphony"))
		return 0;
...
void board_init_f(ulong dummy)
{
	...
	if (board_id == BOARD_ID_TRUX)
		setup_i2c(0, CONFIG_SYS_I2C_SPEED, 0x7f, &i2c_pads_trux);
	else
