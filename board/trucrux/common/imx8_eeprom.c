/*
 * Copyright (C) 2018-2020 Trucrux Ltd.
 *
...
static int trux_scu_eeprom_read(u8 *buf, u32 size)
{
	int ret;
...
int trux_scu_eeprom_read_header(struct trux_eeprom *e)
{
	...
	ret = trux_scu_eeprom_read((uint8_t *)e, sizeof(*e));
	if (ret) {
...
int trux_eeprom_read_header(struct trux_eeprom *e)
{
	...
	/* Probe EEPROM */
	i2c_set_bus_num(TRUX_EEPROM_I2C_BUS);
	ret = i2c_probe(TRUX_EEPROM_I2C_ADDR);
	if (ret) {
#ifdef CONFIG_ARCH_IMX8
		debug("%s: calling SCU to read EEPROM\n", __func__);
		return trux_scu_eeprom_read_header(e);
	...
	/* Read EEPROM header to memory */
	ret = i2c_read(TRUX_EEPROM_I2C_ADDR, 0, 1, (uint8_t *)e, sizeof(*e));
	if (ret) {
		debug("%s: EEPROM read failed ret=%d\n", __func__, ret);
		return ret;
	}
...
int trux_eeprom_get_storage(struct trux_eeprom *ep, int *storage)
{
	/* No data in EEPROM - return default DRAM size */
	if (!trux_eeprom_is_valid(ep)) {
	...
	if (ep->features & TRUX_EEPROM_F_NAND)
...
#elif CONFIG_TARGET_IMX8MN_TRUX_SOM
...
#elif CONFIG_TARGET_IMX8QXP_TRUX_SOM
	printf("\nPart number: VSM-MX8X-%.*s\n", (int)sizeof(partnum), partnum);
#elif CONFIG_TARGET_IMX8QM_TRUX_SOM
				...
				struct dram_cfg_param *table, u8 table_size)
{
	...
	for (i = 0; i < adj_table_size; i++) {
		...
		i2c_read(TRUX_EEPROM_I2C_ADDR, off, 1,
 ...
 * @d - pointer to DRAM configuration structure
  */
void trux_eeprom_adjust_dram(struct trux_eeprom *ep, struct dram_timing_info *d)
{
	int i;
	int *idx_map;
...
#ifdef CONFIG_DM_I2C
int trux_carrier_eeprom_read(int bus_no, int addr, struct trux_carrier_eeprom *ep)
{
	int ret;
	struct udevice *bus;
	struct udevice *dev;
